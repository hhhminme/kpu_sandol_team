...
def lambda_handler(event, context):
    ...
    try:
        ...
        if key[0] == 'weather': #날씨 관련
            ...
            # return_string = Module.Test(time = request_body['action']['detailParams']['current_time']['origin']).arrival_time()
            raise Exception(str(request_body['action']['detailParams']['current_time']['origin']))

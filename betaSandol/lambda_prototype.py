...
def lambda_handler(event, context):
    ...
    try:
        ...
        if key[0] == 'weather': #날씨 관련
            ...
            return_string = "산돌이가 작업을 마무리하지 못했어요ㅠㅠ\n 피드백을 통해 어떤 기능에서 오류가 발생했는지 알려주시면 빠른 시일 내에 작동 하도록 할게요" + str(param[str(json.loads(param[key[0]])["date"]),param[key[1]], param[key[2]], param[key[3]], str(request_body['userRequest']['user']['properties']['botUserKey'])])
